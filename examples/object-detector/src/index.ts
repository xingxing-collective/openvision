...
function displayImageDetections(
  ...
  for (let detection of result.detections) {
    ...
    p.setAttribute("class", "absolute p-0.5 bg-[#007f8b] text-white border-[1px,dashed,rgba(255, 255, 255, 0.7)]");
    const highlighter = document.createElement("div");
    highlighter.setAttribute("class", "bg-[#00ff0040] border-[1px,dashed,#fff] z-10 absolute");
...
window.onload = function () {
  document.querySelectorAll(".detect").forEach((container) => {
    container.querySelector('img')?.addEventListener('click', async function (e) {
